...
- Schema is built from migrations in `supabase/migrations/`, applied **in order** via the Supabase SQL Editor or `supabase db push` (see README):
  ...
  - `0015_cutline_files.sql` — creates the **private `cutline-files` Storage bucket** for the Cutline Generator. No tables: objects are ephemeral working files keyed `{user_id}/{job_id}/{in|out}/{file}`, with one `storage.objects` policy granting each user full CRUD over their own `{user_id}/` folder. See **Cutline Generator**.
...
### Cutline Generator (admin print-prep tool)

- **Admin UI:** `/tools/cutline-generator` (`app/(app)/tools/cutline-generator/`) — inside the `(app)` group, so `requireAdmin()` + `AppShell` are inherited. Drag-drop one or many 1200×1500 JPG/PNG designs and get one **print-ready PDF per image** with a vector cut-contour overlaid; batch with **Download All ZIP**. The client component drives a per-file **uploading → processing → complete / failed** state machine (one route call per file, real upload progress via `XMLHttpRequest`, concurrency 3).
...
- **Reusable presets:** `lib/cutline/presets.ts` (server+client-safe metadata) is the swap point — add a preset `{ id, label, assetPath }` and drop its PDF under `public/assets/cutlines/`. The asset is read with `fs` at runtime, so each preset path must be listed in `outputFileTracingIncludes` in `next.config.ts` (mirroring the invoice-logo entry) to be bundled into the function on Vercel.
- **Server-only processing, Storage round-trip:** route handlers under `app/api/cutline/` — `generate` (auth via `requireAdminApi()` in `lib/cutline/storage.ts`, uploads the source to the private bucket, composes, stores the PDF, returns a short-lived signed URL), `zip` (downloads the job's `out/*.pdf`, bundles with **`jszip`**, returns a signed URL — built+downloaded through Storage to dodge function body caps), and `job` (`DELETE` cleanup of the temp folder on "Start over"). All `runtime = "nodejs"` (sharp/pdf-lib are native). The browser never composes PDFs.
